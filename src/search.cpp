...
struct ISearcher {
    virtual void step(Shape empty_area) = 0;
};
...
class SearcherCRTP : public ISearcher {
    ...
    std::unordered_set<Shape> used_pieces; // canonical forms

    void step_impl(Shape empty_area) {
        if (!empty_area) {
            if (used_pieces.size() < nme.min_n || used_pieces.size() > nme.max_n)
                return;
            if (auto id = nme.name([this](uint64_t v){
                    return used_pieces.contains(Shape{ v });
                }); id)
                static_cast<Derived *>(this)->log(*id);
            return;
        }

        auto recurse = [&,this](Shape sh) {
        ...
        std::unordered_set<Shape> shapes{ empty_area.front_shape() };
        if (1u >= nme.min_m)
            recurse(empty_area.front_shape());
        for (auto m = 1u; m <= nme.max_m; m++) {
            std::unordered_set<Shape> next;
            for (auto sh : shapes)
...
public:
    void step(Shape empty_area) override {
